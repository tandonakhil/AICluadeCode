...
**Owner:** `industry-expert` (executed by `test-agent`)
...
Entry point: `bash tests/suites/industry/run.sh` (delegates to `tests/suites/_runner.sh`,
...
**Test-count delta vs `1b1b56e`: 23 → 23, unchanged.**
...
- Expected: exit 0
- Actual: **exit 0, 23 ran, 0 failed**
- Result: PASS
- Evidence: `scratchpad/p19/suite-industry.txt`; collected node ids under
  `tests/suites/industry/` = 23, matching the executed count exactly

### Scenario: the suite is not empty and not silently shrinking
- Status: EXECUTED
- Expected: a non-zero scenario count, and any drop explained
- Actual: **23 scenarios. 23 → 23, unchanged**
- Result: PASS
