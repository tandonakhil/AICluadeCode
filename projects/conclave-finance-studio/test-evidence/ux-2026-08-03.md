...
**Owner:** `ui-ux-designer` (executed by `test-agent`)
...
Entry point: `bash tests/suites/ux/run.sh` (delegates to `tests/suites/_runner.sh`,
...
**Test-count delta vs `1b1b56e`: 194 → 194, unchanged.**
...
- Expected: exit 0
- Actual: **exit 0, 194 ran, 0 failed**
- Result: PASS
- Evidence: `scratchpad/p19/suite-ux.txt`; collected node ids under
  `tests/suites/ux/` = 194, matching the executed count exactly

### Scenario: the suite is not empty and not silently shrinking
- Status: EXECUTED
- Expected: a non-zero scenario count, and any drop explained
- Actual: **194 scenarios. 194 → 194, unchanged**
- Result: PASS
